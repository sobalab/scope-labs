...
export function ReviewerNotes({ value, locked, onChange }: ReviewerNotesProps) {
  return (
    <div className="space-y-8">
      ...
      {locked ? (
        <p className="rounded-lg border border-border bg-surface-sunk px-12 py-12 text-[13px] leading-[1.55] text-body">
        ...
        <textarea
          ...
          className="w-full resize-y rounded-lg border border-border bg-surface-sunk px-12 py-12 text-[13px] leading-[1.55] text-body placeholder:text-faint focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent-soft"
        />
      )}
    </div>
  );
